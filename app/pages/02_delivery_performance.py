...
BG           = "rgba(0,0,0,0)"
PALETTE      = dict(primary="#F97316", navy="#0F2744",
...
def load_data() -> pd.DataFrame:
    ...
    df["traffic_bin"]   = pd.cut(df["traffic_level_score"],  bins=5, labels=["Very Low","Low","Med","High","Very High"])
    df["weather_bin"]   = pd.cut(df["weather_severity_score"],bins=5, labels=["Clear","Mild","Moderate","Severe","Extreme"])
    return df
...
with st.sidebar:
    ...
    max_dist     = st.slider("Max distance (km)", 1, int(df["delivery_distance_km"].max()), int(df["delivery_distance_km"].max()))
...
def _theme(fig, height=320, **kw):
    fig.update_layout(
        template=PLOT_THEME, height=height,
        paper_bgcolor=BG, plot_bgcolor=BG,
        font=dict(color=FONT_COLOR, size=11),
        margin=dict(l=0, r=0, t=28, b=0), **kw,
    )
...
k1.metric("On-time Rate",       f"{on_time_rate:.1f}%",
...
with r1c1:
    ...
    # Sample for perf — scatter with 15k points is slow
    sample = dff.sample(min(3000, len(dff)), random_state=42)
    fig = px.scatter(
        sample, x="estimated_delivery_time", y="delivery_time_minutes",
        color="delayed_delivery_flag",
        color_discrete_map={0: PALETTE["success"], 1: PALETTE["danger"]},
        labels=dict(estimated_delivery_time="Estimated (min)",
                    delivery_time_minutes="Actual (min)",
                    delayed_delivery_flag="Delayed"),
        opacity=0.55, hover_data=["delivery_distance_km", "tier_label"],
    )
    # Perfect prediction line
    lim = [0, sample[["estimated_delivery_time","delivery_time_minutes"]].max().max() + 5]
    fig.add_shape(type="line", x0=lim[0], y0=lim[0], x1=lim[1], y1=lim[1],
                  line=dict(color=FONT_COLOR, dash="dash", width=1.5))
    fig.add_annotation(x=lim[1]*0.72, y=lim[1]*0.78, text="Perfect prediction",
                       showarrow=False, font=dict(color=FONT_COLOR, size=10))
    chart(fig, height=360,
          legend=dict(title="", orientation="h", yanchor="bottom", y=1.01, xanchor="right", x=1))

with r1c2:
    title("Delivery Time Distribution")
    fig = go.Figure()
    for flag, label, color in [(0,"On-time", PALETTE["success"]), (1,"Delayed", PALETTE["danger"])]:
        sub = dff[dff["delayed_delivery_flag"] == flag]["delivery_time_minutes"]
        fig.add_trace(go.Histogram(
            x=sub, name=label, nbinsx=30, opacity=0.72,
            marker_color=color,
    ...
    chart(fig, height=360,
          ...
          xaxis_title="Delivery Time (min)", yaxis_title="Orders")
...
with r2c1:
    ...
    pivot = (dff.groupby(["order_day_of_week","order_hour"])["delayed_delivery_flag"]
               .mean().mul(100).unstack(fill_value=0)
               .rename(index={i:d for i,d in enumerate(["Mon","Tue","Wed","Thu","Fri","Sat","Sun"])}))
    fig = px.imshow(
        pivot, aspect="auto", text_auto=".0f",
        color_continuous_scale=[[0, HEATMAP_ZERO],[0.5,"#F97316"],[1, PALETTE["danger"]]],
        labels=dict(x="Hour of Day", y="", color="Delay %"),
    )
...
with r3c1:
    ...
    traffic = (dff.groupby("traffic_bin", observed=True)["delivery_time_minutes"]
                  .agg(["mean","std"]).reset_index()
                  .rename(columns={"traffic_bin":"Traffic","mean":"Avg","std":"Std"}))
    fig = go.Figure(go.Bar(
        x=traffic["Traffic"], y=traffic["Avg"],
        error_y=dict(type="data", array=traffic["Std"], visible=True,
                     color=FONT_COLOR, thickness=1.2),
        marker_color=PALETTE["primary"],
        text=traffic["Avg"].round(1), textposition="outside",
...
with r3c2:
    ...
    weather = (dff.groupby("weather_bin", observed=True)["delivery_time_minutes"]
                  .agg(["mean","std"]).reset_index()
                  .rename(columns={"weather_bin":"Weather","mean":"Avg","std":"Std"}))
    colors  = ["#16A34A","#65A30D","#F97316","#EA580C","#DC2626"]
    fig = go.Figure(go.Bar(
        x=weather["Weather"], y=weather["Avg"],
        error_y=dict(type="data", array=weather["Std"], visible=True,
                     color=FONT_COLOR, thickness=1.2),
        marker_color=colors[:len(weather)],
        text=weather["Avg"].round(1), textposition="outside",
...
with r4c1:
    ...
    breakdown = (dff.groupby("tier_label")[["preparation_time_minutes","delivery_time_minutes"]]
                    .mean().reset_index())
    breakdown["transit"] = breakdown["delivery_time_minutes"] - breakdown["preparation_time_minutes"]
    fig = go.Figure()
    for col, label, color in [
        ...
        fig.add_trace(go.Bar(
            x=breakdown["tier_label"], y=breakdown[col],
            name=label, marker_color=color,
        ))
    chart(fig, height=300, barmode="stack",
          yaxis_title="Minutes",
          legend=dict(orientation="h", yanchor="bottom", y=1.01, xanchor="right", x=1))
...
with r4c2:
    ...
    fig = px.scatter(
        sample3, x="delivery_partner_experience_years", y="delivery_efficiency_score",
        color="tier_label",
        color_discrete_map={"Tier 1": PALETTE["navy"],
                            "Tier 2": PALETTE["primary"],
                            "Tier 3": PALETTE["success"]},
        opacity=0.5, size_max=6,
        labels=dict(delivery_partner_experience_years="Experience (yrs)",
...
with r4c3:
    ...
    fig = go.Figure()
    for tier, color in [("Tier 1", PALETTE["navy"]),
                        ("Tier 2", PALETTE["primary"]),
                        ("Tier 3", PALETTE["success"])]:
    ...
    fig.add_hline(y=0, line_dash="dash", line_color=FONT_COLOR, line_width=1,
                  annotation_text="On-time line", annotation_font_color=FONT_COLOR)
    chart(fig, height=300, yaxis_title="Delay Gap (min)", showlegend=False)

st.divider()

# ── ROW 5 — SLA funnel + hourly trend ─────────────────────────────────────────
r5c1, r5c2 = st.columns([1, 2])

with r5c1:
    title("SLA Compliance by City Tier")
    sla = (dff.groupby("tier_label")["delayed_delivery_flag"]
              .agg(total="count", delayed="sum")
              .assign(on_time=lambda d: d["total"] - d["delayed"],
                      rate=lambda d: (1 - d["delayed"]/d["total"]) * 100)
              .reset_index())
    fig = px.bar(
        sla, x="tier_label", y="rate",
        color="rate",
        color_continuous_scale=[[0, PALETTE["danger"]],[0.6,"#F97316"],[1, PALETTE["success"]]],
        text=sla["rate"].apply(lambda v: f"{v:.1f}%"),
        range_y=[0, 105],
        labels=dict(tier_label="", rate="On-time %"),
    )
    fig.update_traces(textposition="outside")
    chart(fig, height=300,
          coloraxis_showscale=False, showlegend=False, yaxis_title="On-time Rate (%)")

with r5c2:
    title("Hourly Avg Delivery Time  (actual vs estimated)")
    hourly = (dff.groupby("order_hour")[["delivery_time_minutes","estimated_delivery_time"]]
                 .mean().reset_index())
    fig = go.Figure()
    for col, label, color, dash in [
        ...
        fig.add_trace(go.Scatter(
            x=hourly["order_hour"], y=hourly[col],
            mode="lines+markers", name=label,
            line=dict(color=color, width=2, dash=dash),
            marker=dict(size=5),
            hovertemplate=f"{label}: %{{y:.1f}} min<extra></extra>",
        ))
    fig.add_vrect(x0=11.5, x1=14.5, fillcolor="rgba(249,115,22,0.08)",
                  line_width=0, annotation_text="Lunch", annotation_position="top left",
                  annotation_font_color=FONT_COLOR)
    fig.add_vrect(x0=18.5, x1=21.5, fillcolor="rgba(249,115,22,0.08)",
                  line_width=0, annotation_text="Dinner", annotation_position="top left",
                  annotation_font_color=FONT_COLOR)
    chart(fig, height=300,

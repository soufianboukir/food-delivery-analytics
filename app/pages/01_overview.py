...
# ── 1. CITY DISTRIBUTION ───────────────────────────────────────────────────────
with c1:
    st.markdown('<p class="section-title">Orders by City Tier</p>', unsafe_allow_html=True)
...
with c2:
    st.markdown('<p class="section-title">On-time vs Delayed</p>', unsafe_allow_html=True)

    delay_df = dff["delayed_delivery_flag"].value_counts().rename({
        0: "On-time",
        1: "Delayed"
    }).reset_index()
...
with c3:
    ...
    hour_day = dff.groupby(["order_day_of_week", "order_hour"]).size().reset_index(name="orders")
    ...
    fig3 = px.imshow(
        pivot,
        color_continuous_scale=[[0, "#F8FAFC"], [1, "#F97316"]],
        aspect="auto"
    )
...
# ── 4. REVENUE BY TIER ─────────────────────────────────────────────────────────
with c4:
    st.markdown('<p class="section-title">Revenue by City Tier</p>', unsafe_allow_html=True)
...
with c1:
    st.markdown('<p class="section-title">Traffic vs Weather Impact</p>', unsafe_allow_html=True)

    heat = dff.groupby(
        ["weather_severity_score", "traffic_level_score"]
    )["delivery_time_minutes"].mean().reset_index()
...
with c2:
    st.markdown('<p class="section-title">Customer Satisfaction Drivers</p>', unsafe_allow_html=True)

...
with st.sidebar:
    ...
    segment_sel = st.multiselect(
        "Customer Type", ["Premium", "Standard"], default=["Premium", "Standard"]
    )
...
st.caption(f"{len(dff):,} Orders | " f"Age {age_range[0]}-{age_range[1]}")
...
c1.metric("Avg Loyalty", f"{dff['customer_loyalty_score'].mean():.1f}")

c2.metric("Premium Share", f"{dff['premium_customer_flag'].mean()*100:.1f}%")

c3.metric("Avg Rating", f"{dff['customer_rating'].mean():.2f}")

c4.metric("Promo Usage", f"{dff['promo_code_used'].mean()*100:.1f}%")

c5.metric("Avg Tip", f"${dff['tip_amount'].mean():.2f}")
...
with col1:
    ...
    fig = px.histogram(
        ...
        barmode="overlay",
        opacity=0.7,
        color_discrete_map={"Premium": PAL["primary"], "Standard": PAL["navy"]},
    )
...
with col1:
    ...
    stages = ["All Orders", "Promo Used", "Completed Orders", "Rating >= 4"]
...
with col2:
    ...
    grp = dff.groupby("loyalty_tier", observed=True)["order_value"].mean().reset_index()

    fig = px.bar(grp, x="loyalty_tier", y="order_value", color="loyalty_tier")
...
with col1:
    ...
    grp = (
        dff.groupby(["age_bin", "premium_label"], observed=True)
        .size()
...
with col2:
    ...
    fig.add_trace(go.Bar(x=tip["loyalty_tier"], y=tip["tip_rate"], name="Tip Rate %"))

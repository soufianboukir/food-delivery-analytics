...
package = joblib.load(
    os.path.join(BASE_DIR, "..", "models", "classification_model_package.pkl")
)
...
def test_predicts_positive_class():
    ...
    assert preds.sum() > 0, (
        "Model predicts no cancellations " "(all predictions are class 0)"
    )
...
def test_model_does_not_degrade():
    ...
    assert accuracy >= BASELINE_ACCURACY - TOLERANCE, (
        f"Model degraded! " f"Accuracy={accuracy:.4f}, " f"baseline={BASELINE_ACCURACY}"
    )

    assert f1 >= BASELINE_F1 - TOLERANCE, (
        f"Model degraded! " f"F1={f1:.4f}, " f"baseline={BASELINE_F1}"
    )

    assert recall >= BASELINE_RECALL - TOLERANCE, (
        f"Model degraded! " f"Recall={recall:.4f}, " f"baseline={BASELINE_RECALL}"
    )

    assert precision >= BASELINE_PRECISION - TOLERANCE, (
        f"Model degraded! "
...
def test_confusion_matrix_has_true_positives():
    ...
    assert tp > 0, "Model detected zero true positives"
